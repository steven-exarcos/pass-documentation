...
| submissionStatus* | String | The current status of the Submission, derived from the [Deposit](deposit.md) status(es), [RepositoryCopy](repository-copy.md) status(es) and the `eventType` of the most recent [SubmissionEvent](submission-event.md) ([_see list below_](#submission-status-options)) |
