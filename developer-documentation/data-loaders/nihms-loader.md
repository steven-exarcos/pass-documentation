...
Once a PACM account is established, generating the token can be done via the [PACM Website](http://www.pubmedcentral.nih.gov/utils/pacm/).
Use the eRA Commons login and sign in with your username and password. Note that the login instructions may be different
for every institution as the authentication mechanism may differ. After logging in, use the link at the top of the page
that is labeled `API Token`. This page will generate a new token every time it is accessed and your new API token will
appear here.
